...
class filters(object):
    ...
    def add_filter(self, name, units='a', grid=True):
        ...
        if isinstance(name, type('')):  # single filter
            if not self.filter_order.count(name):
                ...
                self.nfilters += 1
                self._load_filter(
                    name, units=units, cosmology=self.cosmo, vega=self.vega, solar=self.solar)
            ...
            for i in name:
                if not self.filter_order.count(i):
                    ...
                    self.nfilters += 1
                    self._load_filter(
                        i, units=units, cosmology=self.cosmo, vega=self.vega, solar=self.solar)
        else:
            raise ValueError(
    ...
    def _load_filter(self, fname, units='a'):
        ...
        if self.has_vega:
            if self.vs[fname].min() < self.vega[:, 0].min() or \
               self.vs[fname].max() > self.vega[:, 0].max():
                raise ValueError(
                    'The filter frequency is out of Vega frequency!')
            self.vega_mag[fname] = -1.0 * \
                self.calc_mag(self.vega[:, 0], self.vega[:, 1], 0, fn=fname)
        ...
        if self.has_solar:
            ...
            if self.vs[fname].min() < self.solar[:, 0].min() or \
               self.vs[fname].max() > self.solar[:, 0].max():
                raise ValueError(
                    'The filter frequency is out of Solar frequency!')
            self.solar_mag[fname] = self.calc_mag(
                self.solar[:, 0], self.solar[:, 1], 0, fn=fname)
    ...
    def calc_mag(self, vs, sed, z, fn=None, apparent=False, vega=False, cosmology=None):
        ...
        if apparent:
            app = 5. * np.log10(cosmo.luminosity_distance(z)
        ...
        shifted = vs / (1 + z)
        for i in fn:
            c = ((shifted > self.f_vs[fn].min()) & (
                shifted < self.f_vs[fn].max())).sum()
            if c < 3:
            ...
            if shifted.min() > self.f_vs[fn].min() or shifted.max() < self.f_vs[fn].max():
                print("Warning, wavelength range from SSP models is outside of filter,",
                      " magnitude is assigned nan")
                mag[i] = np.nan

            interp = interp1d(vs, sed)
            sed_flux = (1 + z) * simps(interp(self.f_vs[fn] * (
                1 + z)) * self.f_tran[fn] / self.f_vs[fn], self.f_vs[fn])
            mag[i] = -2.5 * \
                np.log10(sed_flux / self.ab_flux[fn]) + app + to_vega
        return mag
