...
class SSP_models(object):
    ...
    nsample    : The frequency sample points. Default: None, uses the model frequences.
                 Otherwise, the interpolated frequency will be used. For the popurse of
                 reducing required memory.
                 Int number or list, numpy array.
                 If int number, frequency will be assigned equally between min and max of model frequency.
                 If list, or numpy array, it will be directly interpolated with the model frequency.
        ...
        if self.defined_freq is not None:
            if isinstance(self.defined_freq, type(1)):
                for i, mn in enumerate(self.met_name):
                    newfreq = np.arange(
                        self.vs[mn].min(), self.vs[mn].max(), self.defined_freq)
    ...
    def get_seds(self, simdata, dust_func=None, units='Fv'):
        ...
        units      : The units of wavelength. Default: 'Fv'
        ...
        for i, metmodel in enumerate(self.met_name):
            if self.has_masses:
                self.seds[metmodel] /= self.masses[metmodel]
            if dust_func is not None:
                self.seds[metmodel] *= dust_func(
                    self.ages[metmodel], self.ls[metmodel])
            f = interp1d(self.ages[metmodel], self.seds[metmodel])

            if self.nmets > 1:
                ids = mids == i
            else:
                ids = np.ones(mids.size, dtype=np.bool)
